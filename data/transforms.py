    ...
    if cfg.data.augmentation.lower() == 'other':
        transform_train = transforms.Compose([transforms.ToPILImage(),
						      ...
						      transforms.RandomApply([
								  transforms.RandomCrop(cfg.data.input_size, padding=int(0.15 * cfg.data.input_size)),  # 0.15
								  transforms.ColorJitter(brightness=(0.5, 0.9), contrast=(0.5, 2.5), saturation=(0.5, 1.5), hue=(-0.15, 0.15)),
								  transforms.RandomHorizontalFlip(),
								  transforms.RandomVerticalFlip(),
								  transforms.RandomAffine(degrees=(-15, 15), translate=(0.048, 0.048), )], p=0.9),
						      transforms.ToTensor()
				                      		])
    else:
