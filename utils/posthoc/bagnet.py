...
def generate_heatmap_pytorch(model, image, target, patchsize=33, stride=1, SIZE=512, padding=True):
    """
    ...
    with torch.no_grad():
        ...
        if padding:
            ...
            image = padded_image[None].astype(np.float32) # add the batch dimension
            
            # pad_const = int(patchsize/2)
        ...
        # extract patches
        patches = input.permute(0, 2, 3, 1) # numpy format: (h,w,c)
        ...
        return logits.reshape((SIZE, SIZE))


def plot_heatmap(heatmap, original, cmap='RdBu_r', 
    ...
    if original is not None:
        ...
        in_image_upscaled = transform.rescale(original_greyscale, dilation, mode='constant', 
                                              channel_axis=False, anti_aliasing=True)
    ...
    return (heatmap, overlay, extent, cmap, cmap_original, abs_min, abs_max)