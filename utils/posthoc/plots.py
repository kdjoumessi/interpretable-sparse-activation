...
def plot_binary_GradCAM_heatmap(org_img, list_dics, df, y_label, proba=None, plot_prob=False, size=(21, 21), bag_alpha=.25, cam_alpha=.6):
    ...
    with plt.style.context('./plot_style.txt'):
        ...
        for i in range(bs):
            ...
            if plot_prob:
                plt.title('ResNet, $p={:0.3f}$'.format(resnet[i][0][1])) if (j==2) else plt.title('$p={:0.3f}$'.format(resnet[i][0][1]))
            else:
                if (j==2):
                    plt.title('ResNet')            
            ax.axis('off')
            ...
            if plot_prob:
                plt.title('dense BagNet, $p={:0.3f}$'.format(dense[i][0][1])) if (j==3) else plt.title('$p={:0.3f}$'.format(dense[i][0][1]))
            else:
                if (j==3):
            ...
            if plot_prob:
                plt.title('sparse BagNet, $p={:0.3f}$'.format(sparse[i][0][1])) if (j==4) else plt.title('$p={:0.3f}$'.format(sparse[i][0][1]))
            else:
                if (j==4):
...
#------------------------
def plot_multi_class_GradCAM_heatmap(org_img, list_dics, df, size=(21, 21), color_map=False, bag_alpha=.25, cam_alpha=.6, font_size=15):
    '''
        description: plot the image, gradCAM and heatmap(s)
        - org_imgs:
        - list_dics:
    ...
    for i in range(bs):
        ...
        ax.imshow(overlay, extent=extent, interpolation='none', cmap=cmap_2, alpha=bag_alpha)
        if j==2:
            plt.title('ResNet', fontsize=font_size)
        ax.axis('off')
        ...
        for k in range(5):
            ...
            if j==3:
                plt.title('healthy', fontsize=font_size)
            elif j==4:
                plt.title('mild', fontsize=font_size)
            elif j==5:
                plt.title('moderate', fontsize=font_size)
            elif j==6:
                plt.title('severe', fontsize=font_size)
            elif j==7:
                plt.title('proliferate', fontsize=font_size)
            ax.axis('off')
